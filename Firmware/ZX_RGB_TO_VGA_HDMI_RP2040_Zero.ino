/*
Fork of ZX RGB-I to VGA & HDMI by Alex Ekb to RP2040-ZERO 
=======
© 2024 KWF
...
HDMI/VGA output 320 x 240 pixels.

The video output is DVI over HDMI for easy interfacing.

Change log:
-----------
v.0.11
    - added LED workout for RP2040-Zero NeoPixel
    - tested with RP2040-Zero
v.0.1
    - prelimiary modifications for RP2040-Zero board
*/
...
// RP2040-ZERO board definitions
#define BOARD_CODE_ZERO

#ifdef BOARD_CODE_ZERO
  #include <Adafruit_NeoPixel.h> // v0.11 - NeoPixel library added
  // NeoPixel definition
  #define LED_PIN_RGB (16) // GPIO16 for the builtin NeoPixel
  #define NUM_PIXELS (1)
  Adafruit_NeoPixel pixels(NUM_PIXELS, LED_PIN_RGB, NEO_GRB + NEO_KHZ800);
#endif
...
//      //загружаем ранее сохранённые данные заxвата - load previously saved capture data
     memcpy(&capture_setings,flash_data_for_save,sizeof(cap_set_t));
   
//    // memset(&capture_setings,0,sizeof(cap_set_t));//test
//     //подправляем , если в ячйках мусор - we correct if there is garbage in the cells
  check_cap_data(&capture_setings);

// если попадём в настройки после wdt_reboot, то индикатор это покажет - if we get into the settings after wdt_reboot, the indicator will show this
  #ifdef BOARD_CODE_ZERO
    pixels.begin(); // Neopixel
    pixels.setPixelColor(0, pixels.Color(5, 0, 0)); // LED set to red
    pixels.show();
  #else
    ...
    if (watchdog_caused_reboot()) { 
            ...
            bool is_save=false;
            //цикл с разбором команд - cycle with command parsing
            while(1) {
                // sleep_ms(1);
...
//обработчик загрузки параметров захвата - capture parameters loading handler
#define CAP_SET_LOAD(x,T) {if(s_key[0]=='r' || s_key[0]=='w') {  if(s_key[0]=='w') (x)=static_cast<T>(s_data); check_cap_data(&capture_setings); printf("%s %d\n",s_key,(x)); continue;};};
                //смещение изображения - image shift
            ...
            //сохранение параметров - save parameters
            if (is_save) {
                check_cap_data(&capture_setings);
  ...
  // сбрасываем индикаторный  - reset indicator LED
  #ifdef BOARD_CODE_ZERO 
    pixels.setPixelColor(0, pixels.Color(0, 5, 0)); // LED set to green
    pixels.show();
  #else
