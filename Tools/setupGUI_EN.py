...
#
# FeeeSimpleGUI library required.
#
# ---------------------------------------------------------
# Version: 2.0 - English by KWF
# ---------------------------------------------------------
# Change log:
# 2.0 EN:   due to licencing policy of PySimpleGUI library,
#           replaced with FreeSimpleGUI
##
...
def popup(title, filename, message, width=70):
    ...
    sg.Window(title, layout, keep_on_top=True, modal=True, font='Arial 14', auto_close_duration=3).read(close=True, timeout=3000)

message = """ZX RGB-I to VGA & HDMI Configuration Tool
(c) Aleks Ekb & KWF ver. 2.0 EN""".strip()

popup('About', 'pics/ZXHDMI.png', message)
...
if len(dev_TTY)==0:
    ...
    sg.popup_no_buttons(" No serial port found! ",auto_close=True)
    #exit(0)
    sys.exit()
...
invBlock=sg.Frame(layout=[[
     ...
     sg.CBox('K', size=(3, 1),key='is_inv_VS'),
     sg.CBox('S', size=(3, 1),key='is_inv_HS'),
...
sliderDelay1=sg.Frame(layout=[[
    ...
    ]],title=" rising edge ",title_location=sg.TITLE_LOCATION_TOP,key='-SLIDER_DELAY_BLOCK1-')

sliderDelay2=sg.Frame(layout=[[
     sg.Slider((0, 31), orientation='h', enable_events=True, key='-SLIDER_DELAY_2-')          
    ]],title=" falling edge ",title_location=sg.TITLE_LOCATION_TOP,key='-SLIDER_DELAY_BLOCK2-')
...
window = sg.Window('ZX RGB-I to VGA & HDMI ConfigGUI by Alex Ekb v2.0 EN', layout, font=('',12), margins=(10,0), size=(600,650), default_element_size=(40, 1),scaling=True, grab_anywhere=False)
#window.set_icon(red_icon)
#window.set_icon(r'HDMI.png')
...
while True:
    ...
    if event=='btn1Click':
        ...
        sg.popup_no_buttons(" RP2040 reboot in progress ... WAIT ... ", auto_close=True, no_titlebar=True, text_color='Red', font='Arial 16 italic bold', background_color='Yellow')
        l_mode='write'
    #print(btn1.tex)
    #print(values)
            ...
            try: 
                ...
                if (hw_mode.find("mode 0")>0):
                    ...
                    if l_mode=='write':
                        ...
                        sg.popup_no_buttons(" Saving config ... please wait ... ", auto_close=True, no_titlebar=True, text_color='Blue', font='Arial 12 italic bold', background_color='White')
                    ...
                    if l_mode=='read':
                        ...
                        sg.popup_no_buttons(" Reading config ... please wait ... ", auto_close=True, no_titlebar=True, text_color='Blue', font='Arial 12 italic bold', background_color='White')
