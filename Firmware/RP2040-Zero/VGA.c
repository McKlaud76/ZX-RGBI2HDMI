...
//базовые уровни сигналов - basic signal levels
...
//пока выделим память для шаблонов строк с запасом по 3кб на линию - for now, let's allocate memory for line templates with a reserve of 3 KB per line
...
void startVGA()
{
    //инициализация палитры - palette initialization
    for(int i=0;i<16;i++)
    {
    ...
    if (is_wide)
    {
        fdiv=clock_get_hz(clk_sys)/(31468750.0/2);//частота VGA для широкоэкранного режима - VGA frequency for widescreen mode
        line_size=500;
        HS_SHIFT=410;
        HS_SIZE=60;
    ...
    {
        fdiv=clock_get_hz(clk_sys)/(25175000.0/2);//частота VGA по умолчанию - default vga frequency
        line_size=400;
        HS_SHIFT=328;
        HS_SIZE=48;
    ...
    // кадровая синхра - frame synchronization
    ...
    //инициализация PIO - initialize PIO
    //загрузка программы в один из PIO - load program into one of PIO
    uint offset = pio_add_program(PIO_VGA, &pio_program_VGA);
    uint sm=SM_VGA;

    for(int i=0;i<8;i++){gpio_init(beginVGA_PIN+i);
    gpio_set_dir(beginVGA_PIN+i,GPIO_OUT);pio_gpio_init(PIO_VGA, beginVGA_PIN+i);};//резервируем под выход PIO - reserve for PIO output
  
    pio_sm_set_consecutive_pindirs(PIO_VGA, sm, beginVGA_PIN, 8, true);//конфигурация пинов на выход - pin configuration for output
    ...
    sm_config_set_fifo_join(&c, PIO_FIFO_JOIN_TX);//увеличение буфера TX за счёт RX до 8-ми - increasing TX buffer at the expense of RX to 8
    ...
    PIO_VGA->sm[sm].clkdiv=div32&0xfffff000; //делитель для конкретной sm - divisor for specific sm
    //инициализация DMA - DMA initialization
   
    dma_chan_ctrl = dma_claim_unused_channel(true);
    int dma_chan = dma_claim_unused_channel(true);
    //основной ДМА канал для данных - main DMA channel for data
    ...
    //канал DMA для контроля основного канала - DMA channel for main channel control
