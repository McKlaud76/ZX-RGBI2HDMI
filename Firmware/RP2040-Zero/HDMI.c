...
//количество буферов ДМА можно увеличить, если будет подёргивание - the number of DMA buffers can be increased if there is twitching
...
static void __not_in_flash_func(dma_handler_HDMI)() {
    ...
    else
    {
        if ((y>=490) && (y<492))
        {
            //кадровый синхроимпульс - frame sync pulse
...
void startHDMI()
{
    ...
    sleep_ms(10);
    //инициализация констант - constants initialization
    ...
    //инициализация PIO - PIO initialization
    ...
    //настройка side set - side set setting
    ...
    pio_sm_set_consecutive_pindirs(pio_HDMI0, sm_HDMI0, beginHDMI_PIN_data, 6, true);//конфигурация пинов на выход - pin configuration for output
